...
let triggerQuestionsLength = computed(() =>
  triggerQuestions.value.length ? triggerQuestions.value.length : 0
);
...
let chooseEmoji = (emojiAnswer: number) => {
  if (!sureToLeaveIsShown.value) {
    triggerStore.permanentAnswers.push(
      warrior.value.answers[currentQuestion.value] == emojiAnswer
    );
    let duration = 100;
    emojiAnswer
...
onClickOutside(triggerModal, () => {
  setTimeout(() => {
    closingModal(true, false);
  }, 100);
});
...
let destroySureToLeave = () => {
  setTimeout(() => {
    sureToLeaveIsShown.value = false;
  }, 100);
};
...
<template>
  ...
  >
    ...
    <button
      class="closing-modal-button"
      @click.prevent="closingModal(true, false)"
    >
      X
    </button>
    <div class="warrior-card">
      <div>
        ...
        <p class="trigger-count">
          {{
            currentQuestion + 1 > triggerQuestionsLength
              ? triggerQuestionsLength
              : currentQuestion + 1
          }}
        </p>
      </div>
  ...
  <div class="sure-to-leave-background">
    <div v-if="sureToLeaveIsShown" ref="sureToLeave" class="sure-to-leave">
      <h4>Вы уверены, что хотите выйти?</h4>
      <p>Результат не сохранится</p>
      <div id="stl-buttons">
        <button
          id="stl-buttons-yes"
          @click.prevent="closingModal(false, false)"
        >
          Да
        </button>
        <button id="stl-buttons-no" @click.prevent="destroySureToLeave">
          Нет
        </button>
      </div>
    </div>
