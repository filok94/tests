  ...
  actions: {
    async getTriggerGame() {
      ...
      onValue(triggerInfo, async (snapshot) => {
        try {
          ...
          this.warriorCards = await data.card;
          this.warriorCards.forEach(() => {
    ...
    async postResults() {
      ...
      try {
        ...
        await set(
          fireRef(db, Pathes.toUserTriggerAnswers),
          this.triggerAnswersResults
        );
    ...
    computeAnswers(payload: number | null) {
      let rightAnswersCount = this.permanentAnswers.filter(
        (e) => e == true
      ).length;
      payload != null
    ...
    changePerson(toPerson: TriggerPerson) {
